-- request_voyage_booking inserted profile_id = auth.uid() without ever ensuring a matching
-- row exists in public.profiles. Customer accounts only get a profiles row once they save
-- their settings on /profile (update-my-profile edge function), so any user who books before
-- visiting that page hit "violates foreign key constraint voyage_booking_requests_profile_id_fkey".
-- Auto-provision a minimal profile row from auth.users on first booking instead.
