...
> **Stato (23/09/2026).** **A è implementata** e applicata (`20260923095529_voyage_recorded_tracks.sql`): stato reale, scostamenti da questo design e dettagli in `Wiki/29 - Tracciati Reali GPX.md`. Differenze principali: niente tabelle `voyage_track_points`/`voyage_track_stops` (il file originale nel bucket è la sorgente, le soste stanno nel segmento), granularità a **segmento** (una tratta può avere più file), riconciliazione e metriche calcolate nel browser dell'admin, import consentito anche su tratte non ancora registrate come concluse (solo avviso in UI) perché i viaggi storici non hanno tratte. **B** è stata realizzata in forma leggera sul biglietto esistente (`VoyageTicketCard`: miglia misurate, rotta prevista/reale, velocità, soste) — restano da fare snapshot immutabile, seriale, renderer Satori e pagina pubblica.
