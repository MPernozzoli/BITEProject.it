...
- **Mail:** `AdminMail.tsx` vive tra le pagine admin perché integra lista, dettaglio e compose su API Vercel. La UI privilegia nome mittente, oggetto e preview del testo nuovo rispetto ai badge tecnici, usa l'indirizzo solo come fallback e collassa le citazioni nel corpo mail con controlli espandi/nascondi. Accetta anche query `compose=1&to=...&subject=...` per aprire direttamente la modale di scrittura da altre superfici admin.
