...
- **Editoriale:** `AdminEditorialPlan`, `AdminEditorialPlanSettingsDialog`, `AdminEditorialPlanSlotDialog` (piano editoriale) → `apps/web/src/lib/editorial-plan.ts`. Il calendario ora include un cockpit social mensile con slot/target/reach/engagement, indicatori insight nelle celle e una modale social per creare asset, gestire stato/caption dei target e registrare snapshot `editorial_post_insights` con metriche e note qualitative.
