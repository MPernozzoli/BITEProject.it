...
| Articoli | `article_search`, `article_get`, `article_create_draft`, `article_update`, `article_translate`, `article_seo_optimize`, `article_schedule`, `article_unschedule`, `article_upload_image`, `article_media_upload_url`, `article_attach_media` |
...
`src/server/mcp/markdown.ts` converte nei due sensi fra Markdown e il ProseMirror JSON dell'editor. Non usa `generateJSON` di TipTap perché le estensioni del client tirano dentro React e un DOM: costruisce l'albero dai token di `marked`. Copre la formattazione che offre l'editor — grassetto, corsivo, sottolineato (`<u>`), codice inline e a blocco, titoli h1-h3, liste puntate/numerate, tabelle GFM, citazioni, separatori, link — e i media: `![alt](url)` diventa un'immagine, `![alt](url "didascalia")` (sintassi Markdown standard per il titolo) diventa un nodo `mediaFigure` con didascalia, lo stesso con cui l'editor mostra foto captionate; il marcatore `{ai}` in coda al titolo (`![alt](url "didascalia {ai}")`) accende il flag "generata con AI" del nodo, e la lettura lo riscrive, così il round-trip non lo perde. Un link YouTube **da solo in un paragrafo** (`[video: didascalia](https://youtu.be/…)`, la stessa forma che produce la lettura) diventa un `mediaFigure` di tipo youtube; l'URL viene normalizzato in `https://www.youtube-nocookie.com/embed/<id>` (`youtubeEmbedUrl`), perché il nodo lo mette tale e quale in un iframe e un link `watch?v=` non si caricherebbe. Un link YouTube dentro una frase resta un link. Le tabelle hanno la prima riga resa come intestazione (`th`) e si usano dai tool articoli con la normale sintassi Markdown a barre. L'unico nodo che Markdown non rappresenta (mini-mappa) resta restano resi in sola lettura come testo riconoscibile. Coperto da test di round-trip in `src/test/mcp-markdown.test.ts`.
...
## Foto
`src/server/mcp/media.ts` porta un'immagine nel bucket **`logbook-media`** — lo stesso dell'upload manuale in `ArticleEditor.tsx` e in `AdminNewsletterManager.tsx` — da tre sorgenti, perché un agente non ha un form multipart:

- **URL https** (`source_url`): scaricata server-side e ripubblicata, invece di lasciare l'articolo a puntare verso un dominio esterno e fragile. 12MB.
- **Byte in base64** (`data_base64`, base64 puro o data URL): per una foto allegata in chat o generata dal modello. Max **3MB decodificati**: l'intera richiesta JSON-RPC passa dalla function Vercel, che rifiuta corpi oltre 4,5MB.
- **Upload firmato** (`article_media_upload_url` → PUT → `article_attach_media` con `uploaded_path`): per file più grandi o su disco. Il tool rilascia un URL firmato di Supabase Storage (2 ore) e i byte non passano dalla function: `curl -X PUT -H "Content-Type: image/jpeg" --data-binary @foto.jpg "<upload_url>"`. Il bucket non ha limiti di tipo o peso (`allowed_mime_types` e `file_size_limit` null), quindi `article_attach_media` riscarica il file e lo verifica prima di collegarlo; se non è un'immagine **lo rimuove** dal bucket pubblico.

In tutti i casi il formato si decide dai **byte** (magic number: JPG, PNG, WEBP, GIF, AVIF — `sniffImageType`), non dal content-type dichiarato; solo per `source_url` l'header remoto resta come ripiego. `newsletter_upload_image` e `voyage_waypoint_upload_image` usano lo stesso modulo (solo da URL).

**Collegare il media all'articolo.** `article_upload_image` con `article_id` + `attach_as`, oppure `article_attach_media` per un media già disponibile (file firmato, URL del bucket, URL esterno — ripubblicato — o video YouTube), lo mettono direttamente dove lo mette l'editor, con la stessa semantica:

| `attach_as` | Effetto | Cartella default |
| --- | --- | --- |
| `cover` | `cover_image`, ritaglio riportato al centro (50/50, zoom 1) salvo `cover_focal_x/y`/`cover_zoom` | `covers` |
| `body` | `mediaFigure` nel corpo IT e/o EN (`body_language`), in coda, in testa o dopo un titolo (`after_heading`, `after_heading_en`), con `caption_*`/`alt_*` per lingua e `ai_generated`. Accetta anche YouTube | `articles` |
| `instagram_story_it` / `_en` | `instagram_story_image_*` e `instagram_story_use_cover_* = false` | `instagram-stories/it\|en` |

Bilinguismo: se una didascalia arriva in una sola lingua, l'altra resta vuota e il tool lo **segnala** invece di copiarla. I controlli che non richiedono il file (articolo esistente, titolo trovato, niente video come cover) avvengono **prima** del caricamento, così un errore non lascia file orfani nel bucket. Coperto da `src/test/mcp-article-media.test.ts`.
...
`article_create_draft`/`article_update` non toccano solo il corpo: cover con punto focale (`cover_focal_x/y`, percentuale **0-100**, 50 = centro) e zoom (`cover_zoom`, 1-2.5) — la stessa scala di `src/lib/article-cover.ts`: fino a settembre 2026 lo schema MCP la descriveva come 0-1, che in pagina spostava il ritaglio nell'angolo in alto a sinistra —, immagini delle Instagram Story (`instagram_story_image_it/en`, `instagram_story_use_cover_it/en`), slug bilingui SEO (`slug_it`/`slug_en`, indicizzati unique case-insensitive — un duplicato torna come errore DB leggibile), tag (per nome: risolti su quelli esistenti o creati al volo, **sostituiscono** l'elenco esistente quando passati, non lo sommano — stessa semantica "cancella e riscrivi" di `ArticleEditor.tsx`), autori (`{profile_id, role}`, stessa sostituzione, il profilo deve già esistere), collegamento voyage/waypoint/segmento, posizione (`location_name`/`latitude`/`longitude`). `article_get` li restituisce tutti, tag inclusi per nome.
