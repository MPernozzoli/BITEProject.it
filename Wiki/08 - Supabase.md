...
- `20260924095309_admin_bookings_apply_directly_and_comped.sql` — gli admin in viaggio sono staff: `request_voyage_booking` crea la loro prenotazione già `user_confirmed` e `is_comped`; `user_propose_voyage_booking_legs` (admin sul proprio booking) e `admin_propose_voyage_booking_legs` (booking di admin o equipaggio) applicano subito le tratte tramite l'helper interno `apply_voyage_booking_legs_directly` (audit `auto_accepted`, nessuna email). Entrambe le RPC di proposta accettano le tratte già sul booking anche se non più prenotabili (già navigate). Backfill `is_comped` sulle prenotazioni admin attive → [[13 - Booking Voyage]]
- `20260924100735_booking_deposit_deferred_and_repeated_balance_reminders.sql` — colonna `deposit_deferred_at` (acconto rinviato al saldo), gate di `admin_set_voyage_booking_status` aggiornato, `voyage_booking_balance_reminder_due` + `enqueue_voyage_booking_balance_reminders` con promemoria ripetuti e importo residuo → [[11 - Pagamenti Bunq]]
- `20260924095440_profile_contact_phone.sql` — telefono come dato **privato** del profilo: tabella `profile_contact_details` (1:1 con `profiles`, `phone_country_code` `+<1-4 cifre>`, `phone_number` 4-14 cifre), leggibile solo dal proprietario e dagli admin, nessun grant ad `anon`. Non è una colonna di `profiles` perché quella tabella è leggibile pubblicamente (anche da anon). Il trigger `sync_profile_phone_after_booking_request_write` (AFTER INSERT / UPDATE OF `candidate_info` su `voyage_booking_requests`) copia `candidate_info.phoneCountryCode/phoneNumber` sul profilo, normalizzando; un valore malformato viene ignorato, mai sollevato → [[13 - Booking Voyage]]
...
- `profile_contact_details` — telefono del profilo (prefisso + numero), privato: proprietario e admin. Alimentato dalla pagina `/profile` (upsert diretto) e dal trigger sulle candidature; precompila il campo telefono dei form di candidatura → [[13 - Booking Voyage]]
...
- `admin_propose_voyage_booking_legs` (RPC admin) — registra una proposta di tratte alternative in `voyage_booking_plan_changes`, con messaggio admin opzionale in metadata, senza modificare direttamente la matrice Gantt; se il booking è di un admin o dell'equipaggio applica invece la modifica subito → [[13 - Booking Voyage]]
