...
|---|---|---|
| `request.ts` | POST | crea request-inquiry Bunq, ricalcola importo, salva `voyage_booking_deposits`, ritorna link `bunq.me`. `409` se importo > €500 |
| `status.ts` | GET | fallback polling: ri-verifica lo stato del pagamento per `bookingRequestId` |
| `webhook.ts` | POST | riceve callback Bunq alla liquidazione |
| `bank-transfer.ts` | — | flusso alternativo bonifico bancario (per importi oltre limite Bunq) |
