...
- `voyage/VoyageJoinPanel.tsx` — l'intero flusso "Partecipa" dentro la sezione `#partecipa` di `VoyagePage`: stepper a tre passi sempre visibile, lista tratte selezionabile, barra sticky in fondo alla viewport che porta avanti la candidatura, e le modali di conferma/pagamento già esistenti. Legge da solo candidature attive, `voyage_booking_settings`, lingue profilo e bozza; se l'utente ha già una candidatura sul viaggio mostra lo stato e rimanda a `/bookings` invece di offrirne una seconda → [[13 - Booking Voyage]]
- `voyage/VoyageJoinLegList.tsx` — le tratte come card grandi e tappabili. Fuori dalla modalità selezione informano soltanto; dentro crescono cerchio di spunta, bordo tratteggiato e la scritta "Tocca per scegliere", e le selezionate dichiarano il proprio ruolo (Imbarco / A bordo / Sbarco). Complessità e pericolo sono pill etichettate, non l'indicatore con tooltip: la card **è** un bottone, e un bottone dentro un bottone non è tappabile su telefono.
- `voyage/VoyageJoinDialog.tsx` — passo 2 in modale (full-screen su mobile): quante persone, nota facoltativa e `CandidateInfoForm`, in due schermate con footer fisso a un'unica azione primaria. Passa la mano al `BookingConfirmDialog` esistente per condizioni e pagamento.
...
- `admin/VoyageTracksManager.tsx`, `admin/VoyageTrackEditor.tsx`, `admin/VoyageTrackMap.tsx` — pagina `/admin/tracks`: upload GPX, elenco tracciati, copertura per tratta; editor di riconciliazione (segmenti per tratta, dividi/unisci/escludi, slider e maniglie trascinabili lungo la traccia) e sua mappa MapLibre → [[29 - Tracciati Reali GPX]]
- `voyage/VoyagePlannedVsActual.tsx` — sezione "Previsto vs effettivo" della pagina viaggio (totali, tratta per tratta, profilo velocità recharts); invisibile senza tracciati confermati. `voyage/TrackRouteSketch.tsx` — SVG statico rotta prevista vs reale usato dal biglietto (sopravvive a `html-to-image`). `VoyageRouteHeroMap` accetta `actualTrack` → [[29 - Tracciati Reali GPX]]
- `ui/slider.tsx` rende un cursore per valore: con `value={[a, b]}` è uno slider a intervallo (prima ne rendeva sempre uno).
