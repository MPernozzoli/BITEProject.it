...
const MapLoadingPlaceholder = ({ label, className = "" }: MapLoadingPlaceholderProps) => {
  return (
    <div
      className={`absolute inset-0 flex items-center justify-center bg-[linear-gradient(180deg,rgba(255,255,255,0.42),rgba(243,246,247,0.62))] dark:bg-[linear-gradient(180deg,rgba(26,37,55,0.42),rgba(20,30,46,0.62))] ${className}`.trim()}
      role="status"
      aria-live="polite"
      aria-label={label}
    >
      <div className="pointer-events-none w-[min(18rem,calc(100%-2rem))] rounded-[20px] border border-glass-edge/65 bg-glass/80 px-4 py-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] backdrop-blur-sm">
