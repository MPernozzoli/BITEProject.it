-- Wire editorial-readiness-alert into the existing pg_cron invocation pattern.

-- 1. Extend invoke_editorial_edge_function to support editorial-readiness-alert
...
-- 2. Schedule hourly cron for editorial-readiness-alert (runs at minute 15 past each hour)
select cron.schedule(
  'editorial-readiness-alert-hourly',
  '15 * * * *',
  $$select public.invoke_editorial_edge_function('editorial-readiness-alert');$$
);
