...
create table if not exists public.observation_parameters (
  ...
  scale_min numeric,
  scale_max numeric,
  color_ramp text not null default 'sequential',
  ...
  -- sequential = single-hue light->dark ramp; cyclic = wrap-around scale, for angles only.
  constraint observation_parameters_color_ramp_check check (color_ramp in ('sequential', 'cyclic')),
...
comment on column public.observation_parameters.scale_min is
  'Lower bound of the plausible range for this variable. The map colours sequential parameters by the range actually present in the filtered data; this bound is the fixed scale for cyclic parameters (angles) and the fallback when the data holds a single distinct value.';
...
-- Internal helper: the ingest path calls it through the trigger, nobody needs it over PostgREST.
revoke all on function public.resolve_voyage_for_timestamp(timestamptz) from public, anon, authenticated;
...
revoke all on function public.observations_set_voyage() from public, anon, authenticated;
...
revoke all on function public.observation_parameters_rebuild_export() from public, anon, authenticated;
...
values
  ('sst', 'degc', 'Sea surface temperature', 'Temperatura superficiale del mare', '°C', 'numeric', 10, 30, 'sequential', 10, '±0.2 °C',
  ...
  ('air_temp', 'degc', 'Air temperature', 'Temperatura dell''aria', '°C', 'numeric', 0, 40, 'sequential', 20, '±0.3 °C',
   'Ambient air temperature from a shielded sensor on deck.',
   'Temperatura dell''aria ambiente da sensore schermato in coperta.'),
  ('humidity', 'pct', 'Relative humidity', 'Umidità relativa', '% RH', 'numeric', 30, 100, 'sequential', 30, '±3 % RH',
   'Moisture content of the air at the measurement location.',
   'Contenuto di umidità dell''aria nel punto di misura.'),
  ('pressure', 'hpa', 'Barometric pressure', 'Pressione barometrica', 'hPa', 'numeric', 980, 1040, 'sequential', 40, '±1 hPa',
   'Atmospheric pressure at sea level.',
   'Pressione atmosferica al livello del mare.'),
  ('wind_speed', 'kt', 'Wind speed', 'Velocità del vento', 'kt', 'numeric', 0, 40, 'sequential', 50, '±5 %',
  ...
  ('dissolved_oxygen', 'mgl', 'Dissolved oxygen', 'Ossigeno disciolto', 'mg/L', 'numeric', 4, 12, 'sequential', 70, '±0.2 mg/L',
  ...
  ('salinity', 'psu', 'Salinity', 'Salinità', 'PSU', 'numeric', 33, 40, 'sequential', 80, '±0.1 PSU',
  ...
  ('plankton_density', 'indm3', 'Plankton density', 'Densità di plancton', 'ind/m³', 'numeric', 0, 5000, 'sequential', 90, 'Net tow, ±20 %',
  ...
  ('sea_state', 'douglas', 'Sea state', 'Stato del mare', 'Douglas scale', 'categorical', null, null, 'sequential', 100, 'Crew observation',
